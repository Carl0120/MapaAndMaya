...
public class CourseService : ICrudService<Course, CourseViewModel>
{
    ...
    public async Task<ActionResult<Course>> Create(CourseViewModel model)
    {
        ...
        var existAny2 = await _dbContext.Courses.AnyAsync(p =>
            p.AcademicCourseId == model.AcademicCourseId && p.StudyPlanId == model.StudyPlanId &&
            p.DegreeModalityId == model.DegreeModalityId);
        ...
        try
        {
            var entityEntry = _dbContext.Courses.Add(entity);
