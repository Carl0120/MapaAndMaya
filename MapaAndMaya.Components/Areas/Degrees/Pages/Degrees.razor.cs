...
public partial class Degrees
{
    ...
    {
        _isLoading = true;
        ItemsCollection = await _degreeService.Find();
        _isLoading = false;
    }
    ...
    private async void AddFormSubmit()
    {
            ...
            ActionResult<Degree> response =  await _degreeService.Create(DegreeViewModel);
            if (response.Status)
            { 
    ...
    private  void NotifyErrors( string title , List<string> errors)
    {
        ...
        foreach (var error  in errors) 
          _notificationService.Notify(new NotificationMessage() { Severity = NotificationSeverity.Error, Summary = title, Detail = error, CloseOnClick = true, Duration = 5000, Style = "width: 400px;" });
